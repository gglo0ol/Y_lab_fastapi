...
Про тесты: всего 4 теста, как в postman'е. Первый тест (test_menu_crud) - тест меню, второй (test_submenu_crud) - тест субменю, третий (test_dishes_crud) - тест блюд, четверты (test_count) - проверка на количество блюд.

1. Клонируем проект из Git. Для этого вводим команду

`git clone https://github.com/gglo0ol/Y_lab_fastapi`

2. Заходим в папку с проектом.
3. \* Для запуска тестов вводим в терминале комманду 

`docker compose -f docker-compose-test.yml up -d && docker logs --follow backend && docker compose -f docker-compose-test.yml down -v`

при этом логи прогона тестов у нас будут отображаться в терминале  

4. Чтобы запустить само приложение с БД пропишем в терминале комманду

`docker compose up -d`

5. Прложение будет доступно по ссылке 

`http://0.0.0.0:8000/docs`

6. Чтобы завершить выполнение программы нужно прописать комманду 
