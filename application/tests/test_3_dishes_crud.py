...
async def test_get_empty_dishes_list(
    client: AsyncClient, saved_data: dict, data_menu_create, data_submenu_create
) -> None:
...
async def test_not_empty_dishes_list(client: AsyncClient, saved_data: dict) -> None:
    menu_id = saved_data["menu"]["id"]
    submenu_id = saved_data["submenu"]["id"]
    check_not_empty_dishes_list = await client.get(
        reverse(get_all_dishes_endpoint, menu_id=menu_id, submenu_id=submenu_id)
    )
    assert (
...
async def test_get_dish(client: AsyncClient, saved_data: dict) -> None:
    menu_id = saved_data["menu"]["id"]
    submenu_id = saved_data["submenu"]["id"]
...
async def test_update_dish(
    client: AsyncClient, saved_data: dict, data_dishes_update: dict
) -> None:
    menu_id = saved_data["menu"]["id"]
    submenu_id = saved_data["submenu"]["id"]
    dish_id = saved_data["dish"]["id"]

    update_dish_response = await client.patch(
        reverse(
            update_dish_endpoint,
            menu_id=menu_id,
    ...
    check_updated_dish = await client.get(
        reverse(
            get_dish_endpoint, menu_id=menu_id, submenu_id=submenu_id, dish_id=dish_id
        )
    )
...
async def test_delete_dish(client: AsyncClient, saved_data: dict) -> None:
    ...
    delete_dish_response = await client.delete(
        reverse(
            delete_dish_endpoint,
            menu_id=menu_id,
            submenu_id=submenu_id,
    ...
    check_dish_response = await client.get(
        reverse(
            get_dish_endpoint, menu_id=menu_id, submenu_id=submenu_id, dish_id=dish_id
        )
    )
