...
def create_submenu(
    menu_id: str, submenu_title: str, submenu_description: str, db: Session
):
    db_submenu = Submenu(
...
def update_submenu_data(
    submenu_id: str, new_submenu_title: str, new_submenu_description: str, db: Session
):
    db_submenu = db.query(Submenu).filter(Submenu.id == submenu_id).first()
    if db_submenu:
        db_submenu.title, db_submenu.description = (
            new_submenu_title,
            new_submenu_description,
        )
        db.add(db_submenu)
        db.commit()
        db.refresh(db_submenu)
        return get_submenu_data(
            db, submenu_id=db_submenu.id, menu_id=db_submenu.menu_id
        )
    else:
