...
def get_submenu(menu_id: str, submenu_id: str, repo: SubmenuService = Depends()):
    return repo.get_submenu_by_id(submenu_id=submenu_id, menu_id=menu_id)


@router.get("/", response_model=List[SubmenuResponse])
def get_all_submenu(menu_id: str, repo: SubmenuService = Depends()):
...
def delete_submenu(menu_id: str, submenu_id: str, repo: SubmenuService = Depends()):
